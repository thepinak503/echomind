...
fn infer_provider(model: &str, args: &Args, config: &Config) -> (String, String) {
    if model.starts_with("gpt") {
        return ("openai".into(), model.to_string());
    }
    if model.starts_with("claude") {
        return ("claude".into(), model.to_string());
    }
    if model.starts_with("gemini") {
        return ("gemini".into(), model.to_string());
    }
    if let Some(i) = model.find('/') {
        let (prov, m) = model.split_at(i);
        return (prov.to_string(), m[1..].to_string());
    }
    (
        args.provider.clone().unwrap_or_else(|| config.api.provider.clone()),
        model.to_string(),
    )
}
