...
impl ApiClient {
        ...
        F: FnMut(&str),
    {
        ...
        let extract = |data: &str| -> Option<String> {
        ...
        while let Some(chunk) = stream.next().await {
            ...
            self.process_sse_lines(&mut buf, &mut full, &mut |text| cb(text), &extract);
        }
        // drain remainder
        self.process_sse_lines(&mut buf, &mut full, &mut |text| cb(text), &extract);
        ...
        F: FnMut(&str),
    {
        ...
        let extract = |data: &str| -> Option<String> {
            serde_json::from_str::<ClaudeStreamEvent>(data)
                .ok()
                .filter(|ev| ev.event_type == "content_block_delta")
                .and_then(|ev| ev.delta.and_then(|d| d.text))
        };
