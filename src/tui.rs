...
/copyq <n>    - Copy question n to clipboard (0=last)
/copyr <n>    - Copy response n to clipboard (0=last)
/copyall      - Copy last question & response together
/quit, /q     - Exit TUI

Keyboard Shortcuts:
Alt+Q         - Copy last question
Alt+R         - Copy last response
Alt+A         - Copy all (last question + response)
Ctrl+C, Ctrl+Q - Exit
  ...
  STREAM:ON/OFF - Streaming status"#;
            ...
            "/logs" => {
                if self.session_log.is_empty() {
                    self.add_message(
                        "System".to_string(),
                        "No session logs available.".to_string(),
                    );
                } else {
            ...
            "/copyq" if parts.len() > 1 => {
                if let Ok(idx) = parts[1].parse::<usize>() {
                    let msg_idx = if idx == 0 {
                        self.messages.len().saturating_sub(2)
                    } else {
                        self.messages.len().saturating_sub(1).saturating_sub(idx * 2)
                    };
                    if msg_idx < self.messages.len() {
                        let question = &self.messages[msg_idx].content;
                        if platform::clipboard::copy_to_clipboard(question).is_ok() {
                            self.add_message(
                                "System".to_string(),
                                format!("✅ Copied question {} to clipboard", idx),
                            );
                            self.log(&format!("Copied question {} to clipboard", idx));
                        } else {
                            self.add_message("System".to_string(), "❌ Failed to copy to clipboard".to_string());
                        }
                    } else {
                        self.add_message(
                            "System".to_string(),
                            format!("No question found at index {}", idx),
                        );
                    }
                } else {
                    self.add_message(
                        "System".to_string(),
                        "Invalid index. Usage: /copyq <n>".to_string(),
                    );
                }
            }
            "/copyr" if parts.len() > 1 => {
                if let Ok(idx) = parts[1].parse::<usize>() {
                    let msg_idx = if idx == 0 {
                        self.messages.len().saturating_sub(1)
                    } else {
                        self.messages.len().saturating_sub(idx * 2 + 1)
                    };
                    if msg_idx < self.messages.len() {
                        let response = &self.messages[msg_idx].content;
                        if platform::clipboard::copy_to_clipboard(response).is_ok() {
                            self.add_message(
                                "System".to_string(),
                                format!("✅ Copied response {} to clipboard", idx),
                            );
                            self.log(&format!("Copied response {} to clipboard", idx));
                        } else {
                            self.add_message("System".to_string(), "❌ Failed to copy to clipboard".to_string());
                        }
                    } else {
                        self.add_message(
                            "System".to_string(),
                            format!("No response found at index {}", idx),
                        );
                    }
                } else {
                    self.add_message(
                        "System".to_string(),
                        "Invalid index. Usage: /copyr <n>".to_string(),
                    );
                }
            }
            "/copyall" => {
                let mut text = String::new();
                let msgs: Vec<_> = self.messages.iter().filter(|m| m.role == "You" || m.role == self.provider.name()).collect();
                for msg in msgs.iter().rev().take(2) {
                    text.push_str(&format!("[{}] {}\n\n", msg.role, msg.content));
                }
                if platform::clipboard::copy_to_clipboard(&text).is_ok() {
                    self.add_message("System".to_string(), "✅ Copied last exchange to clipboard".to_string());
                }
            }
            "/quit" | "/q" => {
                ...
                Event::Key(key) => {
                    if key.modifiers.contains(KeyModifiers::CONTROL) {
                        ...
                        match key.code {
                            ...
                            KeyCode::Char(c) if app.state != TuiState::Processing
                                && app.input.len() < MAX_INPUT_LENGTH =>
                            {
                                app.input.push(c);
                                app.history_index = None;
                            }
                            KeyCode::Backspace if app.state != TuiState::Processing => {
                                app.input.pop();
                                app.history_index = None;
                            }
                            KeyCode::Up if app.state != TuiState::Processing => {
                            ...
                            KeyCode::Down if app.state != TuiState::Processing => {
                                if let Some(idx) = app.history_index {
                            ...
                            KeyCode::Char('q') if key.modifiers.contains(KeyModifiers::ALT) => {
                                if let Some(last_q) = app.messages.iter().rposition(|m| m.role == "You") {
                                    let _ = platform::clipboard::copy_to_clipboard(&app.messages[last_q].content);
                                    app.add_message("System".to_string(), "✅ Copied last question to clipboard".to_string());
                                }
                            }
                            KeyCode::Char('r') if key.modifiers.contains(KeyModifiers::ALT) => {
                                if let Some(last_r) = app.messages.iter().rposition(|m| m.role == app.provider.name()) {
                                    let _ = platform::clipboard::copy_to_clipboard(&app.messages[last_r].content);
                                    app.add_message("System".to_string(), "✅ Copied last response to clipboard".to_string());
                                }
                            }
                            KeyCode::Char('a') if key.modifiers.contains(KeyModifiers::ALT) => {
                                let mut text = String::new();
                                let msgs: Vec<_> = app.messages.iter().filter(|m| m.role == "You" || m.role == app.provider.name()).collect();
                                for msg in msgs.iter().rev().take(2) {
                                    text.push_str(&format!("[{}] {}\n\n", msg.role, msg.content));
                                }
                                if !text.is_empty() {
                                    let _ = platform::clipboard::copy_to_clipboard(&text);
                                    app.add_message("System".to_string(), "✅ Copied last exchange to clipboard".to_string());
                                }
                            }
                            KeyCode::Esc => break,
                            _ => {}
...
fn draw_header(f: &mut Frame, app: &TuiApp, area: Rect) {
    ...
    let stream_indicator = if app.stream {
        "STREAM:ON"
    } else {
        "STREAM:OFF"
    };
    ...
    let content = Line::from(vec![
        ...
        Span::styled(app.provider.name(), Style::default().fg(Color::Green)),
        Span::styled(" | ", Style::default().fg(Color::DarkGray)),
        Span::styled(&app.model, Style::default().fg(Color::Yellow)),
        Span::styled(" | ", Style::default().fg(Color::DarkGray)),
        Span::styled("TEMP:", Style::default().fg(Color::Magenta)),
        Span::styled(
            format!("{:.1}", app.temperature),
            Style::default().fg(Color::White),
        ),
        Span::styled(" | ", Style::default().fg(Color::DarkGray)),
        Span::styled(stream_indicator, Style::default().fg(stream_color)),
        Span::styled(
...
fn draw_messages(
    ...
    for (i, msg) in app.messages.iter().enumerate() {
        ...
        for line in wrapped_content.iter() {
            all_lines.push((line.clone(), Color::White, bg_color));
        }
