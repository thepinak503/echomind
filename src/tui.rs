...
/// Key must be provided via ECHOMIND_ENCRYPTION_KEY env var (hex-encoded 32 bytes).
#[allow(dead_code)]
fn encrypt_data(data: &[u8]) -> Result<Vec<u8>> {
    let key_bytes = get_encryption_key()?;
    let unbound = UnboundKey::new(&AES_256_GCM, &key_bytes)
        .map_err(|_| crate::error::EchomindError::ConfigError("Invalid encryption key".into()))?;
    let key = LessSafeKey::new(unbound);

    let rng = ring::rand::SystemRandom::new();
    let mut nonce_bytes = [0u8; 12];
    ring::rand::SecureRandom::fill(&rng, &mut nonce_bytes)
        .map_err(|_| crate::error::EchomindError::ConfigError("Failed to generate nonce".into()))?;
...
fn decrypt_data(data: &[u8]) -> Result<Vec<u8>> {
    if data.len() < 12 {
        return Err(crate::error::EchomindError::ConfigError("Encrypted data too short".into()));
    }
    let key_bytes = get_encryption_key()?;
    let unbound = UnboundKey::new(&AES_256_GCM, &key_bytes)
        .map_err(|_| crate::error::EchomindError::ConfigError("Invalid encryption key".into()))?;
    ...
    let nonce_bytes: [u8; 12] = data[..12].try_into()
        .map_err(|_| crate::error::EchomindError::ConfigError("Invalid nonce".into()))?;
    let nonce = Nonce::assume_unique_for_key(nonce_bytes);
...
fn get_encryption_key() -> Result<[u8; 32]> {
    let hex_key = std::env::var("ECHOMIND_ENCRYPTION_KEY").map_err(|_| {
        crate::error::EchomindError::ConfigError(
            "ECHOMIND_ENCRYPTION_KEY env var not set. Generate with: openssl rand -hex 32".into(),
        )
    })?;
    let bytes = hex::decode(&hex_key).map_err(|_| {
        crate::error::EchomindError::ConfigError("ECHOMIND_ENCRYPTION_KEY must be valid hex".into())
    })?;
    if bytes.len() != 32 {
        return Err(crate::error::EchomindError::ConfigError(
            "ECHOMIND_ENCRYPTION_KEY must be exactly 32 bytes (64 hex chars)".into(),
        ));
    }
    let mut key = [0u8; 32];
            ...
            match event::read()? {
                ...
                Event::Mouse(MouseEvent { kind, .. }) => match kind {
                    MouseEventKind::ScrollUp => {
                        ...
                        let max_scroll = total_lines.saturating_sub(visible);
                        vertical_scroll =
                            vertical_scroll.saturating_add(SCROLL_LINES).min(max_scroll);
                    }
