...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{
        Block, Borders, List, ListItem, Paragraph, Scroll, Wrap,
    },
    Frame, Terminal,
};
        ...
        if let Ok(response) = rx.try_recv() {
            if !response.is_empty() {
                ...
                if !app.response.is_empty() {
                    app.add_message(app.provider.name(), app.response.clone());
                    app.input.clear();
                    ...
                    match key.code {
                        KeyCode::Enter => {
                            if let AppState::Input = app.state {
                                if !app.input.is_empty() {
                                    ...
                                    processing_task = Some(tokio::task::spawn(async move {
                                        if let Err(e) = process_query(
                                            input, provider, model, temperature, max_tokens, top_p, top_k, stream, config, args, tx_process
                                        ).await {
                                            eprintln!("Error processing query: {:?}", e);
                                        }
                                        let _ = tx_process.send(String::new()); // Signal completion
                                    }));
                                }
                        ...
                        KeyCode::Tab => {
                            ...
                            app.state = match app.state {
                                AppState::Input => AppState::Response,
                                AppState::Response => AppState::Input,
                                AppState::Processing => AppState::Input,
                            };
                        }
                        _ => {}
                    }
