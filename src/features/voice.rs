    ...
    impl VoiceManager {
            ...
            if cfg!(target_os = "macos") {
                ...
                Command::new("arecord")
                    .args(&[
                        ...
                        "cd",
                        "-d",
                        &duration_seconds.unwrap_or(5).to_string(),
                        audio_file,
                    ])
                    .output()
                    .await
            ...
            *recording.lock().unwrap() = false;
            ...
            if let Ok(transcription) = self.transcribe_with_whisper(audio_file).await {
                ...
                Err(EchomindError::Other(
                    "Speech-to-text not available".to_string(),
                ))
            }
        }
        ...
        pub async fn text_to_speech(&mut self, text: &str, _voice: Option<&str>) -> Result<()> {
            let (_stream, stream_handle) = OutputStream::try_default().map_err(|e| {
                EchomindError::Other(format!("Failed to create output stream: {}", e))
            })?;
            ...
            if cfg!(target_os = "macos") {
                Command::new("say")
                    ...
                    .await
                    .map_err(|e| {
                        EchomindError::Other(format!("Failed to generate speech: {}", e))
                    })?;
            } else if cfg!(target_os = "linux") {
                Command::new("espeak")
            ...
            if let Some(ref handle) = self.output_handle {
                let file = File::open(output_file).map_err(|e| {
                    EchomindError::Other(format!("Failed to open audio file: {}", e))
                })?;
            ...
            *self.recording.lock().unwrap() = false;
            if let Some(stream) = &self._stream {
                stream.pause().map_err(|e| {
                    EchomindError::Other(format!("Failed to stop recording: {}", e))
                })?;
            }
            Ok(())
        }
...
mod voice_stub {
    ...
    impl VoiceManager {
        pub fn new() -> Result<Self> {
            Err(EchomindError::Other(
                "Voice support is not enabled. Rebuild with 'cargo build --features voice'"
                    .to_string(),
            ))
        }
        ...
        pub async fn start_voice_input(
            ...
            Err(EchomindError::Other(
                "Voice support is not enabled".to_string(),
            ))
        }

        pub async fn text_to_speech(&mut self, _text: &str, _voice: Option<&str>) -> Result<()> {
            Err(EchomindError::Other(
                "Voice support is not enabled".to_string(),
            ))
        }
...
#[cfg(feature = "voice")]
pub use voice_impl::VoiceManager;

#[cfg(not(feature = "voice"))]
pub use voice_stub::VoiceManager;
