...
impl PerformanceMonitor {
        ...
        while Instant::now() < end_time {
            for _ in 0..concurrent_requests {
                ...
                let handle = tokio::spawn(async move {
                    ...
                    monitor
                        .benchmark_model(&client, &model, &provider, &prompt)
                        .await
                });
            ...
            for handle in handles.drain(..) {
                if let Ok(Ok(benchmark)) = handle.await {
    ...
    fn update_metrics(&mut self, success: bool, response_time: Duration, tokens: u32, cost: f64) {
        ...
        self.metrics.average_response_time =
            total_time.as_secs_f64() / self.request_times.len() as f64;
    ...
    fn get_provider_for_model<'a>(
        ...
        for (provider, client) in api_clients {
            ...
            let matches = (model.starts_with("gpt") && provider == "openai")
                || (model.starts_with("claude") && provider == "claude")
                || provider == "ollama";
            if matches {
    ...
    fn calculate_comparison_metrics(&self, results: &[BenchmarkResult]) -> HashMap<String, f64> {
        ...
        let avg_response_time: f64 = results
            ...
            .sum::<f64>()
            / results.len() as f64;
        let avg_tokens_per_second: f64 =
            results.iter().map(|r| r.tokens_per_second).sum::<f64>() / results.len() as f64;
        let avg_cost: f64 =
            results.iter().map(|r| r.cost_estimate).sum::<f64>() / results.len() as f64;
        ...
        metrics.insert(
            "average_tokens_per_second".to_string(),
    ...
    fn generate_recommendations(&self) -> Vec<String> {
        ...
        if self.metrics.average_response_time > 5000.0 {
            recommendations.push(
                "Slow response times. Consider using faster models or optimizing prompts."
                    .to_string(),
            );
        }
